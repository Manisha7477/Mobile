...
import GPReturnableReviewPhoto from "./GPReturnableReviewPhoto";
import GPReturnableReviewMaterial from "./GPReturnableReviewMaterial";
...
const GPReturnableReview: React.FC = () => {
    ...
    const updateOutwardStatus = async (newStatus: string) => {
        try {
            const updatePayload = {
                outward_id: Number(outward_id),
                returnable_gate_pass_no: formData.returnable_gate_pass_no,
                approved_by: user?.username || "",
                status: newStatus,   // <-- FIXED
                updated_by: user?.username || "",
            };

            await api.put("/api/GatePassRG/UpdateReturnableGatePass", updatePayload);
    ...
    useEffect(() => {
        ...
        api
            ...
            .then((res) => {
                ...
                const formatted = res.data.data.materials.map((m: any, idx: number) => ({
                    id: String(idx + 1),
                    description: m.description,
                    orderedQty: m.actual_quantity,
                    receivedQty: m.received_quantity,
                    returnQty: m.received_quantity,
                    unit: m.unit,
                    remarks: m.remarks,
                    photo: m.goods_photo,
                }));
                ...
                if (data.photos && data.photos.length > 0) {
                    const p = data.photos[data.photos.length - 1]; // LAST PHOTO
                    ...
                    console.log("📸 Latest Final Photo URLs:", mappedPhotos);
    ...
    const handleApprove = async (type: "Pending Approval" | "draft") => {
        try {
            ...
            form.append("status", type === "draft" ? "draft" : "Returnable");
            ...
            /** --------------------------
             * 🟢 PARALLEL PUT API CALL
             ---------------------------*/
            const updatePayload = {
                outward_id: Number(outward_id),
                returnable_gate_pass_no: formData.returnable_gate_pass_no,
                approved_by: user?.username || "",
                status: type === "draft" ? "draft" : "Returnable",
                updated_by: user?.username || "",
            };

            const putPromise = api.put("/api/GatePassRG/UpdateReturnableGatePass", updatePayload);


            /** --------------------------
             * 🟢 MAIN POST API CALL
             ---------------------------*/
            // const postPromise = api.post("/api/GatePassRG/insert-return", form, {
            ...
            <main className="flex-1 overflow-y-auto pb-24">
                <div className="max-w-7xl mx-auto px-3 py-2">
                    <div className="bg-white rounded-lg shadow p-3 space-y-2">
                        ...
                        {/* <GPReturnableReviewPhoto photos={photoDocs} /> */}
                        <GPReturnableReviewPhoto initialPhotos={photos} />
                        <GPReturnableReviewMaterial
                            materials={materials}
            ...
            <footer className="bg-white border-t shadow-md px-3 fixed bottom-0 left-0 right-0">
                <div className="max-w-7xl mx-auto flex justify-end">
                    <ReviewReturnableApprovalAction
                        ...
                        onApprove={() => updateOutwardStatus("Approved")}
                    />
                </div>
            </footer>
        </div>
...
export default GPReturnableReview;
