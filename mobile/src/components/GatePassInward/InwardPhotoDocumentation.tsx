...
interface PhotoDocumentationProps {
  onPhotosChange: (photos: (string | undefined)[]) => void;
}
 
const InwardPhotoDocumentation: React.FC<PhotoDocumentationProps> = ({ onPhotosChange }) => {
  ...
  useEffect(() => {
    onPhotosChange([
      photos[0]?.length ? photos[0][photos[0].length - 1] : undefined,
      photos[1]?.length ? photos[1][photos[1].length - 1] : undefined,
      photos[2]?.length ? photos[2][photos[2].length - 1] : undefined,
      photos[3]?.length ? photos[3][photos[3].length - 1] : undefined,
    ]);
  }, [photos]);
  ...
  return (
    <section className="mb-3">
      ...
      <div className="flex flex-nowrap overflow-x-auto gap-3 pb-2 hide-scrollbar">
...
export default InwardPhotoDocumentation;