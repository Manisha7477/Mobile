...
import MocNextApproverPriview from "./MocNextApproverPreview";
...
const MocNextApproverFormModel: React.FC<NextRequestFormModelProps> = ({
    currentStep,
    handleCancel,
    handleBack,
    handleSubmit,
    ...
    useEffect(() => {
        if (mocData) {
            ...
            setFormData({
                ...
                commentApprover: mocData.approver_comments || "",
    ...
    const updateMocStatus = async (actionType: "reject" | "send_back" | "approve") => {
        ...
        const payload: any = {
            ...
            hira_approved_date: mocData.hira_approved_date,
            sic_approved_date: mocData.sic_approved_date,
            approved_date: mocData.approved_date? mocData.approved_date:new Date().toISOString(),
        ...
        // ✅ Ensure time_limit is a valid date string (YYYY-MM-DD)
    ...
    const mocSections: Array<{ id: SectionKey; title: string }> = [
        ...
        { id: "commentApprover", title: "6. Comment (Approver)" },
    ];

    if (showPreview) {
        return (
            <div className="w-full">
                <MocNextApproverPriview data={formData} onBack={handleBackToForm} />
            </div>
        );
    }
...
export default MocNextApproverFormModel;
