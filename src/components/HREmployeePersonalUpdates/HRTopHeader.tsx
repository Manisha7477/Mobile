...
const HRTopHeader: React.FC<MocHeaderProps> = ({
  title,
  subTitle,
  searchQuery,
  onSearchChange,
  ...
  return (
    ...
    >
      ...
      {showAddButton && onAssetClick && (
        <button
          className="flex items-center justify-center gap-0.5 sm:gap-1 whitespace-nowrap rounded-md bg-white px-1 sm:px-2 md:px-3 py-1 sm:py-1.5 md:py-1.5 text-xs sm:text-xs md:text-sm lg:text-base font-medium text-blue-600 shadow-sm transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-blue-600 flex-shrink-0"
          onClick={onAssetClick}
        >
          <span className="hidden md:inline">{addButtonLabel || 'Asset Declaration'}</span>
          <span className="md:hidden">{addButtonLabel ? addButtonLabel.split(' ')[0] : 'Asset Declaration'}</span>
        </button>
      )}
      {showAddButton && onAddClick && (
        ...
        >
          ...
          <span className="hidden md:inline">{addButtonLabel || 'Add New Employee'}</span>
          <span className="md:hidden">{addButtonLabel ? addButtonLabel.split(' ')[0] : 'Add New Employee'}</span>
        </button>
      )}
    </div>
  )
