...
const ReturnablePhotoDoc: React.FC<PhotoDocumentationProps> = ({ onPhotosChange }) => {
    ...
    useEffect(() => {
        onPhotosChange([
            photos[0].length > 0 ? [...photos[0]] : null,
            photos[1].length > 0 ? [...photos[1]] : null,
            photos[2].length > 0 ? [...photos[2]] : null,
            photos[3].length > 0 ? [...photos[3]] : null,
        ]);
    }, [photos]);
    useEffect(() => {
                    ...
                    >
                        ...
                        {activeCameraIndex === sectionIndex ? (
                            <div>
                                ...
                                />
                                <canvas ref={(el) => (canvasRefs.current[sectionIndex] = el)} className="hidden" />
                            ...
                            <div className="flex gap-2 items-center">
                                <div
                                    onClick={() =>
                                        isMobileDevice() ? fileRefs.current[sectionIndex]?.click() : startCamera(sectionIndex)
                                    }
                                    className="border border-dashed w-12 h-12 rounded-md flex items-center justify-center cursor-pointer text-xl text-gray-500"
                                >
                                    📷
                                </div>
                                <button
                                    onClick={() => startCamera(sectionIndex)}
...
export default ReturnablePhotoDoc;
